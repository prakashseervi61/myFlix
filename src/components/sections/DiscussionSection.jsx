...
const DiscussionSection = ({ movieId, user }) => {
  ...
  return (
    <div className="mt-0 space-y-6">
      ...
      <div className="bg-surface/20 border border-white/5 rounded-2xl overflow-hidden flex flex-col h-[550px] shadow-2xl">
        {/* Messages List */}
              ...
              >
                <div className={`flex items-end gap-3 max-w-[85%] ${msg.userId === user?.id ? 'flex-row-reverse' : 'flex-row'}`}>
                  <div className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-[10px] font-bold ${
                    msg.userId === user?.id ? 'bg-primary text-white shadow-lg shadow-black/40' : 'bg-surface-secondary text-gray-400'
                  }`}>
                  ...
                  <div className={`p-3.5 rounded-2xl text-[13px] sm:text-sm leading-relaxed ${
                    ...
                    <div className="flex items-center gap-2 mb-1.5 pointer-events-none">
                      <span className={`text-[9px] font-black uppercase tracking-widest ${
                        msg.userId === user?.id ? 'text-primary' : 'text-gray-500'
                      }`}>
                        {msg.username}
                      </span>
                    </div>
          ...
          {user ? (
            <form onSubmit={handleSubmit} className="relative">
              <input
                ...
                className="w-full bg-background/80 border border-white/10 rounded-full py-3 pl-5 pr-14 text-white placeholder:text-gray-600 focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all font-medium"
                required
              />
              <button
                type="submit"
                disabled={isSubmitting || !newMessage.trim()}
                className="absolute right-1.5 top-1.5 bottom-1.5 px-4 bg-primary hover:bg-[#a82e25] disabled:bg-[#2A1F25] text-muted/30 text-white rounded-full transition-all active:scale-90 flex items-center justify-center border border-white/10 shadow-lg"
              >
                {isSubmitting ? (
            ...
            <div className="text-center py-2">
              ...
              <Link
                to="/login"
                className="inline-flex items-center gap-2 text-xs font-bold text-primary hover:text-[#a82e25] transition-colors uppercase tracking-wider"
              >
                <LogIn size={14} />
